...
from utils import  KINETICES_CONFIG_PATHS, get_cfg_custom, AverageMeter, OPT_PATH, UCF_MODEL_TO_CKPTS
import argparse
import math
...
def arg_parse():
    ...
    parser.add_argument('--adv_path', type=str, default='UCF-slowfast_resnet101-1-kernel15-uniform-mask_num15-top4', help='the path of adversarial examples.')
...
if __name__ == '__main__':
    ...
    for model_name in ['i3d_resnet101', 'i3d_resnet50', 'slowfast_resnet101', 'slowfast_resnet50', 'tpn_resnet101', 'tpn_resnet50']:
        ...
        ckpt_path = UCF_MODEL_TO_CKPTS[model_name]
        model = get_model(cfg)
        model.load_state_dict(torch.load(ckpt_path)['state_dict'])
        model.cuda()
        model.eval()
        
        preds, labels, top1_avg = reference(model, model_name, files_batch, flag=args.adv_path)
    ...
    info_df.to_csv('results_all_models_prediction_ucf101.csv', index=False)
