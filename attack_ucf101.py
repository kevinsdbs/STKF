...
from utils import KINETICES_CONFIG_PATHS, OPT_PATH, get_cfg_custom, UCF_MODEL_TO_CKPTS
import subprocess
...
def arg_parse():
    ...
    parser.add_argument('--model', type=str, default='slowfast_resnet101',
                        help='i3d_resnet101 | slowfast_resnet101 | tpn_resnet101.')
    ...
    args.adv_path = os.path.join(OPT_PATH, 'UCF-{}-{}-kernel{}-{}-mask_num{}-top{}'.format( \
...
if __name__ == '__main__':
    ...
    # loading cfg
    cfg_path = UCF_MODEL_TO_CKPTS[args.model]
    cfg = get_cfg_custom(cfg_path, args.batch_size)
    cfg.CONFIG.MODEL.PRETRAINED = False

    # loading model.
    ckpt_path = UCF_MODEL_TO_CKPTS[args.model]
    model = get_model(cfg)
    ...
    # dataset_loader = get_dataset('/root/autodl-tmp/STKF/STKF_attack/ucf_all_info.csv', '/root/autodl-tmp/STKF/STKF_attack/used_idxs.pkl', args.batch_size)
    # attack
    params = {'kernlen': (2 * args.L - 1),
              'momentum': args.momentum,
              'kernel_mode': args.kernel_mode}
    attack_method = getattr(attack_methods, args.attack_method)(model, mask_num=args.mask_num, params=params, steps=args.step, top_k=args.top_k)
    ...
    for step, data in enumerate(dataset_loader):
        ...
        print("输入STKF类前val_batch的维度:", val_batch.shape)
        adv_batches = attack_method(val_batch, val_label)
        print('--------------------------------------')
        print(adv_batches.shape)
